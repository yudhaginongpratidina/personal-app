...
import { editPortfolioFormSchema, EditPortfolioFormSchema } from "../validators/edit_portolio.validator"

export default function FormEditPortfolio() {

    const [isError, setIsError] = useState<boolean>(false)
    const [message, setMessage] = useState<string>("")

    const { register, handleSubmit, formState: { errors } } = useForm<EditPortfolioFormSchema>({
        resolver: zodResolver(editPortfolioFormSchema)
    })

    const onSubmit = async (data: EditPortfolioFormSchema) => {
        try {
            console.log(data)
        } catch (error) {
            setIsError(true)
